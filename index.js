  ...
  if (newUserChannel === grindTimeVC && oldUserChannel !== grindTimeVC) {
    ...
    for (let i = 0; i < userData.length; i++) {
      ...
      if (userData[i].userID === newMember.id) {
        ...
        const dayTrackerCheck =
          new Date(userData[i].dayTrackerDay).toDateString() ===
        ...
        const monthTrackerCheck =
          oldMonth === currentMonth && oldYear === currentYear;
      ...
      if (fs.existsSync("userData.json")) {
        ...
        for (const userDatum of userData) {
          if (userDatum.userID === userID) {
            ...
            const checkGrindedToday =
              actualDate.toDateString() === userGrindToday.toDateString();
            ...
            const userProfile = new Discord.MessageEmbed()
              ...
              .setDescription(
                "The following is your Grind Time Information. I will slowly add more stuff into the profile such as **economy**, **items**, and more! Also, if you didn't already known, some Grinders have their own profile! Try to type your name to see if you have one."
              )
              .addFields(
                {
                  name: "Current Streak",
                  value: `\`\`\`${userDatum.streak}\`\`\``,
                  inline: true,
                },
                {
                  name: "Max Streak",
